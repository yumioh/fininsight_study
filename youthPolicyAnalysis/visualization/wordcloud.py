...
def create_wordcloud_comment(tokens_column, font_path, image_path, width=800, height=400, background_color='white', max_words=60):
        # 모든 토큰을 하나의 리스트로 합치기
    #all_tokens = [token for tokens in df['content'] for token in tokens]
     # 모든 토큰을 하나의 리스트로 합치기
    all_tokens = [token for tokens in tokens_column for token in eval(tokens) if len(token) > 1]
...
def create_wordcloud_comment2(tokens_column, font_path, image_path, width=800, height=400, background_color='white', max_words=60):
        # 모든 토큰을 하나의 리스트로 합치기
    #all_tokens = [token for tokens in df['content'] for token in tokens]
     # 모든 토큰을 하나의 리스트로 합치기
    all_tokens = [token for tokens in tokens_column for token in eval(tokens) if len(token) > 1]
